...
Steps:
1. Verify the binary is installed: `command -v rtk` — if missing, instruct the user to run `/ufil:rtk-activate` again after `brew install rtk` (macOS) or `curl -fsSL https://raw.githubusercontent.com/rtk-ai/rtk/refs/heads/master/install.sh | sh`. Do not attempt to install it from inside this command.
