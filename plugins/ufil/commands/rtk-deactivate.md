...
3. On success, delete the sentinel `~/.cache/ufil/rtk-initialized` (use `${XDG_CACHE_HOME:-$HOME/.cache}/ufil/rtk-initialized`) so a future `/ufil:rtk-activate` or SessionStart can re-init cleanly. Use `rm -f` so a missing sentinel isn't an error.
