...
4. If `rtk init --show` says the hook is active but the sentinel is missing (or vice versa), call that out — the user may want to run `/rtk-activate` or `/rtk-deactivate` to resync.
